...
typedef struct __attribute__((packed)) _IterStatEntry_struct
{
	UInt64 dotCount;
	float  peakDensity;
	float  reserved0;
...
__kernel void init_iterators_kernel(
  ...
  randXBuffer[get_local_id(0)]    = ((uint4)((0),(0),(0),(0)));
  randCBuffer[get_local_id(0)]    = 0;
  randSeedBuffer[get_local_id(0)] = entropySeedBuffer[get_global_id(0)];
  
  MWC_seed(randXBuffer+get_local_id(0), randCBuffer+get_local_id(0), randSeedBuffer[get_local_id(0)]);
  
  entropyXBuffer[get_global_id(0)] = randXBuffer[get_local_id(0)];
  entropyCBuffer[get_global_id(0)] = randCBuffer[get_local_id(0)];
  
  iterStatBuffer[get_global_id(0)].dotCount = 0;
  iterStatBuffer[get_global_id(0)].peakDensity = 0.0f;
...
__kernel void reset_iterators_kernel(
  ...
  float2 pos = MWC_rand_float2(randXBuffer+get_local_id(0), randCBuffer+get_local_id(0));
  float2 color = ((float2)((0.5f),(0.5f)));
  
  for(int iter = 0; iter < (int)32; iter++)
  {
    uint rnd = MWC_rand(randXBuffer+get_local_id(0), randCBuffer+get_local_id(0));
    uint bi = chooseRandomBranch(rnd & 0x0000FFFF, fractalInfo->branchCount, branchInfo);                
    iterate(&pos, &color, (rnd>>16), branchInfo+bi, variWeightBuffer+(bi*48), randXBuffer+get_local_id(0), randCBuffer+get_local_id(0)); 
  }
  
  iterPosStateBuffer[get_global_id(0)] = pos;
  iterColorStateBuffer[get_global_id(0)] = color;
}
...
)
{
 ...
 float4 samplePaletteBuffer(float2 v, __global uchar4 paletteBuffer[], uint width, uint height)
{
  ...
  return ((float4)(((float)pix.x / 255.0f),((float)pix.y / 255.0f),((float)pix.z / 255.0f),((float)pix.w / 255.0f)));
}
...
__kernel void iterate_kernel(
  ...
  float2 pos =        iterPosStateBuffer[get_global_id(0)];
  float2 color =      iterColorStateBuffer[get_global_id(0)];
  UInt64 dotCount =   iterStatBuffer[get_global_id(0)].dotCount;
  float peakDensity = iterStatBuffer[get_global_id(0)].peakDensity;
  randXBuffer[get_local_id(0)] = entropyXBuffer[get_global_id(0)];
  randCBuffer[get_local_id(0)] = entropyCBuffer[get_global_id(0)];
  ...
  for(int iter = 0; iter < (int)iterCount; iter++)
  {
    ...
    if(x >= 0 && x < xRes && y >= 0 && y < yRes)
    {
      ...
      float4 mem = (*((accumBuffer + 4*((((y))*xRes) + ((x))) + 2*(((ya&1))) + (((xa&1))))));
      mem.x += sample.x;
      mem.y += sample.y;
      mem.z += sample.z;
      mem.w += 1.0f;
...
__kernel void iterate_kernel_hybrid(
               uint             xRes,
               uint             yRes,
  ...
  for(int iter = 0; iter < (int)iterCount; iter++)
  {
    ...
    dotBuffer[iter * get_global_size(0) + gid] = ((float4)((pos.x),(pos.y),(color.x),(color.y)));
...
__kernel void update_stats_kernel(
  ...
  if(get_global_id(0) == 0)
  {
    ...
    float totalSubPixels = (float)(xRes*yRes*(2*2));
    float density = (float)totalDotCount / totalSubPixels;
    float invPixArea = fabs((fractalInfo->vpsTransform.xa.x)*(fractalInfo->vpsTransform.ya.y) - (fractalInfo->vpsTransform.xa.y)*(fractalInfo->vpsTransform.ya.x));
    float scaleConstant = (64.0f/1.0f)*(invPixArea*(float)(2*2))/(float)totalIterationCount;
 ...
 float4 tonemap(__constant FractalInfo* fractal, float4 rawPix, float scaleConstant)
{
  ...
  float z = native_powr(logPix.w,fractal->invGamma);
